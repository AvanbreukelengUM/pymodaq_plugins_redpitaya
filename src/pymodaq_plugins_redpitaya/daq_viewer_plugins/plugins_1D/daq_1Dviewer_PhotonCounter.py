...
class DAQ_1DViewer_PhotonCounter(DAQ_Viewer_base):
    ...
    * PyMoDAQ >= 4.1.0
    ...
    def commit_settings(self, param: Parameter):
        ...
        if param.name() == 'threshold':
            ...
            self.history = int(self.settings['counting', 'stream_length'] / param.value() * 1000)
            self.times = deque(maxlen=self.history)
            self.rates = deque(maxlen=self.history)
    ...
    def grab_data(self, Naverage=1, **kwargs):
        ...
        axis = Axis('time', units='s', offset=0, #or offset = self.times[0]
                    scaling=scaling,
